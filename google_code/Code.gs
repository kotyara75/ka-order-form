 ...
 * Deployment instructions are in http://pipetree.com/qmacro/blog/2013/10/04/sheetasjson-google-spreadsheet-data-as-json/
 */
...
function getDataRows_(spreadsheet, sheetName, startRowNum) {
  ...
  var endRowNum = sheet.getLastRow() - 1;
  if (endRowNum < startRowNum) endRowNum = startRowNum-1;
  var endColNum = sheet.getLastColumn();
  return sheet.getRange(startRowNum, 1, endRowNum , endColNum).getValues();
}
...
function test1() {
  runTest_({
      parameters : {
        id : "1GBabQ_MakwaQX0E1apd3A9HhPjNKhPYDHBf1r3uOAwo",
        sheet : "Note"
      }
  });
}
